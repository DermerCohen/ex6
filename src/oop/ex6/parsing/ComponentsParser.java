...
public class ComponentsParser {
    ...
    private final static String VARIABLE = "^\\s*(final )?\\s*" +
            "(int |boolean |String |char |double ).*;\\s*$";
    private final static String VAR_MOD = "^\\s*(\\w+)\\s*(=)\\s*(.+)(;)$";
    private final static String METHOD_CALL = "^(.*)(\\()(.*)(\\))(;)\\s*$";
    private final static String IF_OR_WHILE = "^\\s*(if|while)\\s*" +
            "(\\()(.*)(\\))\\s*(\\{)\\s*$";
    ...
    private final static int METHOD_CALL_PARAM = 3;
     ...
     */
    public static MainBlock createExpressions (ArrayList<String> lines)
                                               throws invalidSyntax {
...
//        VariableChecks varChecker = new VariableChecks();
        ...
        while (lineCounter < lines.size()){ //'read' all the lines in the file
            ...
            //if the line match a method declaration:
                ...
                if (!mainBlock.methods.containsKey(methodName)) { //if there isnt another
                    ...
                    newMethod = methodChecker.methodParamValidityCheck
                            (inputParam,newMethod,mainBlock);
     ...
     * @throws invalidSyntax
     */
    private static void varModeChecker(BasicBlock block, VariableChecks varChecker,
                                       Matcher varModMatcher)
            throws invalidSyntax {
        ...
        if (foundFirstVariables.isEmpty()) {
            ...
            if (!(foundSecondVariables.isEmpty())) {
                ...
                for (Variable firstVar : foundFirstVariables) {
                    ...
                    for (Variable secondVar : foundSecondVariables) {
                        ...
                        if (VariableFactory.possiblePairs(firstType, secondType)) {
                            ...
                            if (firstVar.isFinal == false &&
                                    secondVar.initialized == true) {
                                return;
                            }
                        }
                    }
                ...
                for (Variable foundFirstVariable : foundFirstVariables) {
                    boolean check = varChecker.valueValidityCheck
                            (value, foundFirstVariable.type);
                    if (check == true) {
     ...
     * @throws invalidSyntax
     */
    public static void blockParser(MainBlock mainBlock, BasicBlock block,
                                   ArrayList<String> lines, int
            ...
            //if the current line is a beginning of a if/while block:
                ...
                for (String exp : expressions) {
                    ...
                    if (!(searchVariable(exp, block, BOOLEAN))) {
                        ...
                        boolean valueCheck =
                                VariableChecks.valueValidityCheck(exp, BOOLEAN);
                        if (valueCheck == false){ //TODO fix this!!
                            ArrayList<Variable> possibleExistingVars =
                                    findVariable(exp,block);
                            if (!possibleExistingVars.isEmpty()) {
                                for (Variable possibleVar : possibleExistingVars){
                                    boolean findVar = VariableFactory.
                                            possiblePairs(possibleVar.type,BOOLEAN);
                                    if (findVar == false ||
                                            possibleVar.initialized == false){
                                        throw new invalidSyntax();
                ...
                //parser the block:
                ComponentsParser.blockParser
                        (mainBlock, newBlock, lines, innerLineCounter, blockEnd);
     ...
     */
        public static ArrayList<Variable> findVariable (String variableName,
                                                        BasicBlock block){
     ...
     * @throws invalidSyntax
     */
        private static boolean searchVariable (String givenString, BasicBlock block,
                                               String givenType) throws invalidSyntax{
     ...
     * @throws invalidSyntax
     */
    private static void legalMethodCall(MainBlock mainBlock, String methodName,
                                        String parametersUsage,
        ...
        if (mainBlock.methods.containsKey(methodName)){
            ...
            if (foundMethod.initialParam.size() != paramUsage.length){
                ...
                for (int i=0; i< foundMethod.initialParam.size(); i++){
                        ...
                        //like the parameter:
                         for (Variable existingVar : exitingVariables){
                             if (existingVar.initialized){
                                 boolean pairCheck = //check if there are from the same
                                         //type
                                         VariableFactory.possiblePairs(existingVar.type,
                                                 foundMethod.initialParam.get(i).type);
                                 if (pairCheck){
                                     break;
                                 } else {
                                     throw new CodeException(CodeException.INVALID_PARAMS_DECLARE);
                    ...
                    } else { //check if the value is from the correct type as the method
                        //required
                        boolean newParamCheck = VariableChecks.valueValidityCheck
                                (curParam,foundMethod
     ...
     * @throws invalidSyntax
     */
    private static void returnCheck (ArrayList<String> lines, int beginIndex,
                                     int endIndex) throws
            invalidSyntax {
