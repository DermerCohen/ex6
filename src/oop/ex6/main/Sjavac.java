...
public class Sjavac {
    ...
    public static void main(String[] args) throws IOException, invalidSyntax {
            try {
                ArrayList<String> lines = FileToArrayParser.convertFileToArray
                        (args[FILE_INDEX]); //convert the file into ArrayList
                MainBlock mainBlock = ComponentsParser.createExpressions(lines);//create
                //the main block that hold the main scope (using the parser)
                ...
                for (String methodKey : methodsKeysSet) {
                    ...
                    ComponentsParser.blockParser
                            (mainBlock, curMethod, lines, curMethod.start, curMethod.end);
                }
